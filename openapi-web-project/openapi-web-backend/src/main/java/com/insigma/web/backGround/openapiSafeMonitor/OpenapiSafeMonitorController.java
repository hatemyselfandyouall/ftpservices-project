    ...
    public ResultVo updateVisitNum(@RequestBody OpenapiDictionaryUpdateVO updateVO) {
      ...
      try{
          ...
          AddLogUtil.addLog(ServletAttributes.getRequest(),loginComponent.getLoginUserName(),loginComponent.getLoginUserId()
                  ,"更新访问次数","更新值为:"+updateVO.getValue(), DataConstant.SYSTEM_NAME,"安全管控",sysOperatelogRecordFacade);
      }catch (Exception e) {
        ...
        if(!interfaceDetailVO.getWhereWord().isEmpty()){
            if(interfaceDetailVO.getWhereWord().get("requester_ip") != null) { //根据ip条件查询
                if(!ips.isEmpty()) { //查询出来黑名单列表不为空
                    ...
                    if (interfaceDetailVO.getWhereWord().get("is_abnormal") != null) { //是否异常条件查询 0-异常 1-正常
                        if ("0".equals(interfaceDetailVO.getWhereWord().get("is_abnormal"))) {   //异常查询  大于等于预警值为异常
                            map.put("condition", " where r.requester_ip like '%" + interfaceDetailVO.getWhereWord().get("requester_ip") + "%' and r.dayCallTimes >= " + Integer.parseInt(openapiDictionaryFacade.getValueByCode(DataConstant.VISIT_NUM)));
                        }
                        if ("1".equals(interfaceDetailVO.getWhereWord().get("is_abnormal"))) {  //正常查询   小于预警值为正常
                            map.put("condition", " where r.requester_ip like '%" + interfaceDetailVO.getWhereWord().get("requester_ip") + "%' and r.dayCallTimes < " + Integer.parseInt(openapiDictionaryFacade.getValueByCode(DataConstant.VISIT_NUM)));
                        }
                ...
                if(interfaceDetailVO.getWhereWord().get("is_abnormal") != null) { //是否异常条件查询 0-异常 1-正常
                    if("0".equals(interfaceDetailVO.getWhereWord().get("is_abnormal"))){   //异常查询  大于等于预警值为异常
                        ...
                        map.put("condition", " where r.dayCallTimes >= " + Integer.parseInt(openapiDictionaryFacade.getValueByCode(DataConstant.VISIT_NUM)));
                    }
                    if("1".equals(interfaceDetailVO.getWhereWord().get("is_abnormal"))){  //正常查询   小于预警值为正常
                        if(!ips.isEmpty()) { //查询出来黑名单列表不为空
                            map.put("condition", " where r.requester_ip not in (" + ips + ") and r.dayCallTimes < " + Integer.parseInt(openapiDictionaryFacade.getValueByCode(DataConstant.VISIT_NUM)));
...
//         }
           ...
           AddLogUtil.addLog(ServletAttributes.getRequest(),loginComponent.getLoginUserName(),loginComponent.getLoginUserId()
                   ,"安全监控查询","安全监控查询", DataConstant.SYSTEM_NAME,"安全管控",sysOperatelogRecordFacade);
       }catch (Exception e) {
