          ...
          if(tableInfo.getTableDatas() != null && tableInfo.getTableDatas().size()>0){
             for(Object tableData : tableInfo.getTableDatas()){
                ...
                if(tableDataJson.getDouble("avgDurationTimes") > dataConfig.getAverageCallTime()){
                    ...
                    WarningStr = WarningStr+"接口id:"+tableDataJson.getLong("interface_id") + ",平均调用时长："+tableDataJson.getDouble("avgDurationTimes")+"<br/>";
                    }
                 }
              }
           }
           if(!"".equals(WarningStr)) {
               sendEmailFacade.sendEmail("huaw@epsoft.com.cn", "开发平台服务监控接口平均调用时长报警", "你好!<br/>以下接口：<br/>" + WarningStr + "已达到报警指标，请及时处理。<br/> 祝：工作顺利！ <br/>" + new Date());
           }
        }
     }
}
